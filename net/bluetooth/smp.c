...
static u8 smp_cmd_pairing_rsp(struct l2cap_conn *conn, struct sk_buff *skb)
{
	...
	/* Just works */
	memset(conn->tk, 0, sizeof(conn->tk));
       if (rsp->io_capability == SMP_IO_KEYBOARD_ONLY &&
               rsp->auth_req == SMP_AUTH_MITM) {

               tk = (random32() % SMP_PASSKEY_MAX);
               conn->hcon->hdev->smp_passkey = tk;
               BT_DBG("sending passkey gen %06d", tk);

               tk = swab32(tk);
              memcpy((conn->tk + 12), &tk, 4);

              hci_notify(conn->hcon->hdev, SMP_PASSKEY_GEN);
      }
...
int smp_sig_channel(struct l2cap_conn *conn, struct sk_buff *skb)
{
	...
	BT_DBG("command code 0x%2.2x", code);
