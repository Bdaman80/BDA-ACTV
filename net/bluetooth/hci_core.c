...
struct wake_lock idletimerlock;
struct timer_list wakeLockTimer;
...
int hci_dev_open(__u16 dev)
{
	...
	if (!ret) {
		...
		/*init the wakelock for active ACL connection*/
		wake_lock_init(&idletimerlock, WAKE_LOCK_SUSPEND,
							"idletimerlock");

		/*Create a timer for wakelock*/
		setup_timer(&wakeLockTimer, waklock_timeout_callback,
							(unsigned long)0);
	} else {
		/* Init failed, cleanup */
...
static int hci_dev_do_close(struct hci_dev *hdev)
{
	...
	wake_lock_destroy(&idletimerlock);
    del_timer(&wakeLockTimer);
...
int le_scan(struct hci_dev *hdev, u8 enable)
{
...
void le_scan_timeout(unsigned long arg)
{
...
static inline void hci_link_tx_to(struct hci_dev *hdev, __u8 type)
{
	...
	/*BT_ERR("%s ACL tx timeout", hdev->name); Old code */
...
int set_le_scan_param(struct hci_dev *hdev, u8 type, u16 interval,
								u16 window)
{
