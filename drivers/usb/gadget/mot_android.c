...
static struct device_pid_vid mot_android_vid_pid[MAX_DEVICE_TYPE_NUM] = {
	{"msc", MSC_TYPE_FLAG | CDROM_TYPE_FLAG, 0x22b8, 0x41c5,
	...
	{"cdrom", CDROM_TYPE_FLAG, 0x22b8, 0x41de, "Motorola CDROM Device",
	 ...
	 USB_CLASS_PER_INTERFACE },
	{"msc_adb", ETH_TYPE_FLAG | MSC_TYPE_FLAG | ADB_TYPE_FLAG, 0x22b8,
				 0x41C7, "Motorola Config 64",
	 USB_CLASS_PER_INTERFACE, USB_CLASS_PER_INTERFACE,
	 USB_CLASS_PER_INTERFACE },
	...
	{"msc_eth", MSC_TYPE_FLAG | ETH_TYPE_FLAG, 0x22b8, 0x41c6,
	"Motorola Config 63"},
...
static int enable_android_usb_product_function(char *device_name, int cnt)
{
	...
	if (!strncmp(device_name, "msc_eth", cnt - 1)) {
		list_for_each_entry(f, &android_config_driver.functions, list) {
			 ...
			 size_t count, loff_t *ppos)
{
	...
	if (!vers_check) {
		...
		if (copy_from_user(vers_info, buffer, count)) {
			printk(KERN_DEBUG "%s - Error copying version \
				buffer\n", __func__);
			return -EFAULT;
		}
		vers_info[count - 1] = 0;
