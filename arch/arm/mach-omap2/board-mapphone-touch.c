		...
		struct i2c_board_info *i2c_info)
{
	...
	if (touch_pwr_en_gpio >= 0) {
		err = gpio_request(touch_pwr_en_gpio, "legacy_touch_enpwr");
		if (err >= 0) {
			err = gpio_direction_output(touch_pwr_en_gpio, 1);
			if (err < 0) {
				printk(KERN_ERR "%s: Unable to config enpwr.\n",
						__func__);
				retval = err;
				goto legacy_qtouch_gpio_init_fail;
			}
		} else {
			printk(KERN_ERR "%s: Pwr_en GPIO request failed.\n",
					__func__);
			retval = err;
			goto legacy_qtouch_gpio_init_fail;
		}
