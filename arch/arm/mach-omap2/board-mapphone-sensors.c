...
#define MAPPHONE_MSP430_RESET_GPIO      129
#define MAPPHONE_MSP430_TEST_GPIO    126
...
/* G2 is not using proximity sensor and we also don't want GPIO 180 initialized
...
{
	struct regulator *reg;
	printk(KERN_DEBUG "mapphone_msp430_initialization\n");
	reg = regulator_get(NULL, "vhvio");
	if (IS_ERR(reg))
		return PTR_ERR(reg);
...
struct msp430_platform_data mapphone_msp430_data = {
	...
	.gpio_reset = MAPPHONE_MSP430_RESET_GPIO,
	.gpio_test = MAPPHONE_MSP430_TEST_GPIO,
	.gpio_int = 0,
...
{
	int err = 0;
	mapphone_msp430_data.gpio_test = MAPPHONE_MSP430_TEST_GPIO;
	mapphone_msp430_data.gpio_int = get_gpio_by_name("msp430_int");

	printk(KERN_DEBUG "Gault MSP 430 Initialization\n");
	/* configure MSP430 gpio */
	err = gpio_request(mapphone_msp430_data.gpio_reset, "msp430 reset");
	gpio_direction_output(mapphone_msp430_data.gpio_reset, 0);
	err = gpio_request(mapphone_msp430_data.gpio_test, "msp430 test");
	gpio_direction_output(mapphone_msp430_data.gpio_test, 0);
	err = gpio_request(mapphone_msp430_data.gpio_int, "msp430 interrupt");
	gpio_direction_input(mapphone_msp430_data.gpio_int);
}
...
/* No Vibrator on G2 and need to disable initializing of Y4_34XX_GPIO181
	linear_vib_only = 0;
